...
void showAll (int count) {
	for (int j = 1; j <= count; j++) { // Displays every title and author in a single line
		cout << bookTitle[j] << " (" << bookAuthor[j] << ")" << endl;
	}
}
...
int showBooksByAuthor (int count, string name) { // Lets the user search books by author Returns all titles and authors
	...
	for (int i = 1; i <= count; i++) {
		if(bookAuthor[i].find(name) != string::npos) { // Searches the string for another string
			cout << bookTitle[i] << " (" << bookAuthor[i] << ")" << endl; // If the searched for string is found in the searched string then prints
...
int showBooksByTitle (int count, string title) {
	int numByTitle;
	for (int i = 1; i <= count; i++) {
		if (bookTitle[i].find(title) != string::npos) { // searches the string for another string
			cout << bookTitle[i] << " (" << bookAuthor[i] << ")" << endl; // If the searched for string is found in the searched string returns that string
